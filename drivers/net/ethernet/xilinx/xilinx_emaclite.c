...
//chn debug
	u8 *pByteData = skb->data;
	if((0x88 == pByteData[16]) && (0xB8 == pByteData[17]))
	{
		pr_info("recv %d bytes net packet:\n", len);

		for(dwLoop=0; dwLoop<len; dwLoop++)
		{
			printk("%x ",skb->data[dwLoop]);
		}
		printk("\n");
		
		//send packet
//		xemaclite_send_data(lp, skb->data, len);
//		pr_info("send %d bytes net packet:\n", len);
	}
...
static int xemaclite_of_probe(struct platform_device *ofdev)
{
	...
	dev_info(dev, "dt set MAC address is %pM\n", ndev->dev_addr);
